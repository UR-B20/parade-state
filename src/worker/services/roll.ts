...
export function toPersonDto(p: PersonnelRow): PersonDto {
  return { id: p.id, unitId: p.unitId as UnitId, rank: p.rank, name: p.name, serviceNo: p.serviceNo, postedInDate: p.postedInDate, postedOutDate: p.postedOutDate };
}
...
export async function createPerson(db: Db, unitId: string, input: { rank: string; name: string; serviceNo?: string | null; postedInDate?: IsoDate }, createdBy: string, now: Date): Promise<PersonDto> {
  const [row] = await db
    .insert(personnel)
    .values({ unitId, rank: input.rank, name: input.name, serviceNo: input.serviceNo ?? null, postedInDate: input.postedInDate ?? sgDateOf(now), createdBy })
    .returning();
  return toPersonDto(row!);
}

export async function updatePerson(db: Db, unitId: string, personId: string, patch: { rank?: string; name?: string; serviceNo?: string | null; postedOutDate?: IsoDate | null }): Promise<PersonDto> {
