...
export function paradeStateXlsx(summary: BattalionSummaryDto, abs: AbsenteesDto, generatedAt: Date): Uint8Array {
  ...
  const header = ['Unit', 'Strength', 'Present', 'MC', 'LL', 'MA', 'RSI', 'Others', 'Status', 'Submitted at'];
  ...
  const summaryRows: Cell[][] = [
    ...
    [],
    header,
    ...units.map((u) => [u.unit.name, u.counts.strength, u.counts.present, u.counts.mc, u.counts.ll, u.counts.ma, u.counts.rsi, u.counts.others, stateText(u.submission), submittedAt(u.submission)]),
    ['Battalion', summary.totals.strength, summary.totals.present, summary.totals.mc, summary.totals.ll, summary.totals.ma, summary.totals.rsi, summary.totals.others, `${summary.unitsSubmitted} of ${summary.unitsTotal} submitted`, ''],
  ];
  return buildXlsx([
    { name: 'Summary', rows: summaryRows, boldRows: [0, 3, summaryRows.length - 1], columnWidths: [14, 10, 10, 8, 8, 8, 8, 8, 30, 14] },
