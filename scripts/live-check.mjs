...
await softly('unit trends', async () => { r = await api(`/units/COY1/attendance/${am.id}/trends`, { token: cdr }); check('unit trends', r.status === 200 && r.body.units[0].onTime === 1 && r.body.days[13].counts.present === 1, JSON.stringify(r.body.units[0])); });
r = await api(`/units/COY2/attendance/${am.id}`, { token: cdr }); check('commander cannot read another unit', r.status === 403, String(r.status));
await softly('S1 summary shows Coy 1 submitted 1 of 2', async () => { r = await api(`/admin/summary/${am.id}`, { token: admin }); const coy1 = r.body.units.find((u) => u.unit.id === 'COY1'); check('S1 summary shows Coy 1 submitted 1 of 2', r.body.unitsSubmitted === 1 && coy1.counts.present === 1 && coy1.counts.mc === 1 && coy1.submission.kind === 'SUBMITTED', JSON.stringify(coy1.counts)); });
await softly('absentees list the MC', async () => { r = await api(`/admin/absentees/${am.id}`, { token: admin }); check('absentees list the MC', r.body.total === 1 && r.body.groups[0].items[0].name === 'Smoke Absent'); });
await softly('battalion trends live day', async () => { r = await api(`/admin/trends/${am.id}`, { token: admin }); check('battalion trends live day', r.body.days[13].live === true && r.body.days[13].unitsSubmitted === 1, JSON.stringify(r.body.days[13].counts)); });
...
await softly('Excel export', async () => { const x = await api(`/admin/export/${am.id}.xlsx`, { token: admin, raw: true }); const xb = new Uint8Array(await x.arrayBuffer()); check('Excel export', x.status === 200 && xb[0] === 0x50 && xb[1] === 0x4b && xb.length > 3000, `${x.headers.get('content-type')} ${xb.length} bytes`); });
await softly('CSV export with BOM', async () => { const x = await api(`/admin/export/${am.id}.csv`, { token: admin, raw: true }); const cb = new Uint8Array(await x.arrayBuffer()); check('CSV export with BOM', x.status === 200 && cb[0] === 0xef && cb[1] === 0xbb && new TextDecoder().decode(cb).includes('Smoke Absent'), `${cb.length} bytes`); });
