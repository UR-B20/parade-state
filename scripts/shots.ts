...
await admin.getByRole('button', { name: 'Sign in as S1 Branch admin' }).click();
await admin.getByRole('heading', { name: '15C4I Battalion' }).waitFor({ timeout: 20_000 });
await admin.getByText('Headline').waitFor({ timeout: 20_000 });
await admin.waitForTimeout(900);
await shot(admin, '19-admin-overview');
await admin.screenshot({ path: `${out}/19b-admin-overview-full.png`, fullPage: true });
await admin.getByRole('tab', { name: /^Branches\/Coy/ }).click();
...
await admin.getByText('S1 Branch admin', { exact: true }).first().waitFor();
...
await awide.getByRole('button', { name: 'Sign in as S1 Branch admin' }).click();
await awide.getByText('Headline').waitFor({ timeout: 20_000 });
await awide.waitForTimeout(900);
await awide.screenshot({ path: `${out}/27a-admin-overview-wide.png`, fullPage: true });
console.log('saved 27a-admin-overview-wide');
await awide.getByRole('tab', { name: /^Branches\/Coy/ }).click();
